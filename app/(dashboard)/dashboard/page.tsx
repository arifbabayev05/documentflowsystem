...
export default function DashboardPage() {
    const { user, can, isLoading: authLoading } = useAuth(); // rename isLoading to avoid conflict if any (but we used correct one now)
    ...
    const fetchCustomers = async (isInitial = false) => {
        if (isInitial) {
            ...
            if (rows.length === 0) setLoadingData(true);
        }
    ...
    const handleSave = async (specificRow?: CustomerRow) => {
        ...
        setIsSaving(true);
        try {
            if (specificRow && specificRow.id) {
                await updateCustomer(specificRow.id, specificRow, user?.email);
            } else {
    ...
    return (
        <AuthGuard>
            <div className="max-w-[1400px] mx-auto space-y-6 animate-in fade-in duration-700 pb-16 relative">
                ...
                <div className="sticky top-0 z-20 bg-bg-main/80 backdrop-blur-md -mx-4 md:mx-0 px-4 md:px-0 pt-2 pb-4">
                    <div className="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border border-border-soft soft-shadow flex flex-col lg:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4 w-full lg:w-auto">
                            <div className="relative w-full lg:w-[450px]">
                                ...
                                <input
                                    type="text"
                                    placeholder="Axtarış..."
                                    className="w-full pl-14 pr-12 py-4 bg-bg-main/50 rounded-[1.2rem] md:rounded-[1.5rem] border border-transparent focus:border-primary/20 focus:bg-white outline-none text-sm font-bold transition-all shadow-inner"
                                    value={searchTerm}
                        ...
                        <div className="flex items-center gap-3 w-full lg:w-auto justify-stretch md:justify-end">
                            {can("customers_create") && (
                                <button
                                    onClick={addRow}
                                    className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-primary text-white px-5 py-4 rounded-2xl font-black text-[11px] uppercase tracking-wider hover:translate-y-[-2px] hover:shadow-xl transition-all active:scale-95 shrink-0"
                                >
                                    <Plus size={20} className="stroke-[3px]" />
                                    <span className="hidden sm:inline">Yeni Sətir</span>
                            ...
                            {(can("customers_create") || can("customers_update")) && (
                                <button
                                    onClick={() => handleSave()}
                                    disabled={isSaving}
                                    className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-text-main text-white px-5 py-4 rounded-2xl font-black text-[11px] uppercase tracking-wider hover:translate-y-[-2px] hover:shadow-xl transition-all active:scale-95 disabled:opacity-50"
                                >
                ...
                {/* Main Table */}
                <div className="bg-white rounded-[1.5rem] md:rounded-[3rem] border border-border-soft soft-shadow overflow-hidden">
                    ...
                    {/* Footer Info */}
                    <div className="p-6 md:p-8 bg-bg-main/30 flex flex-col md:flex-row items-center justify-between border-t border-border-soft gap-4">
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 md:gap-10 text-[11px] font-black text-text-soft uppercase tracking-widest">
                            <div className="px-5 py-2.5 bg-white rounded-2xl border border-border-soft shadow-inner flex items-center gap-3">
                                ...
                                <span>CƏMİ: <span className="text-primary text-sm ml-1">{rows.length}</span> SƏTİR</span>
                            </div>
                            <div className="hidden lg:flex items-center gap-2 font-bold opacity-70 bg-white px-4 py-2.5 rounded-2xl border border-dashed border-border-soft">
                                <Upload size={14} />
                                Excel-dən məlumatları toplu kopyalayıb yapışdıra bilərsiniz
                            </div>
                        </div>
                    </div>
                </div>
