        ...
        <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500 pb-24 px-4 sm:px-6 relative">
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                            {/* Stats */}
                            <div className="flex items-center gap-3 px-6 py-2.5 bg-gray-50 rounded-xl border border-border-soft">
                                <Users size={18} className="text-text-soft" />
                                <span className="text-sm font-bold text-text-main whitespace-nowrap">
                                    Ümumi İstifadəçi : <span className="font-black text-primary">{users.length}</span>
                                </span>
                            </div>
                            ...
                            <div className="relative min-w-[280px]">
                                ...
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-2.5 bg-white border border-border-soft rounded-xl text-sm font-bold outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary/20 transition-all"
                                    value={searchTerm}
                            ...
                            <div className="relative">
                                <select
                                    className="appearance-none pl-4 pr-10 py-2.5 bg-white border-2 border-primary rounded-xl text-sm font-black text-text-main outline-none focus:ring-4 focus:ring-primary/5 transition-all cursor-pointer"
                                    value={roleFilter}
                            ...
                            {/* Permissions Filter Button & Popover */}
                            <div className="relative">
                                <button
                                    onClick={() => setIsFilterOpen(!isFilterOpen)}
                                    className={cn(
                                        "flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all border-2",
                                        permissionFilterIds.length > 0
                                            ? "bg-primary border-primary text-white"
                                            : "bg-white border-transparent shadow-sm text-text-main hover:bg-gray-50 border-border-soft"
                                    )}
                                >
                                    <Filter size={18} />
                                    İcazələr {permissionFilterIds.length > 0 && `(${permissionFilterIds.length})`}
                                </button>

                                {isFilterOpen && (
                                    <>
                                        <div className="fixed inset-0 z-10" onClick={() => setIsFilterOpen(false)} />
                                        <div className="absolute right-0 top-full mt-2 w-[300px] bg-white border border-border-soft rounded-2xl shadow-2xl z-20 py-4 animate-in fade-in slide-in-from-top-2 duration-200">
                                            <div className="px-6 py-2 border-b border-border-soft mb-2">
                                                <span className="text-[10px] font-black text-text-soft/60 uppercase tracking-widest">İCAZƏLƏRƏ GÖRƏ FİLTR</span>
                                            </div>
                                            <div className="max-h-[400px] overflow-y-auto px-2 custom-scrollbar">
                                                {["ADMİN PANELİ İCAZƏLƏRİ", "İSTİFADƏÇİ PANELİ İCAZƏLƏRİ"].map(group => (
                                                    <div key={group} className="pt-2">
                                                        <div className="px-4 py-2 border-t border-border-soft/30 first:border-0 first:pt-0">
                                                            <span className="text-[9px] font-black text-text-soft/40 uppercase tracking-[0.15em]">{group}</span>
                                                        </div>
                                                        {AVAILABLE_PERMISSIONS.filter(p => p.group === group).map(p => (
                                                            <label key={p.id} className="flex items-center gap-3 px-4 py-2 hover:bg-gray-50 rounded-xl cursor-pointer group transition-colors">
                                                                <input
                                                                    type="checkbox"
                                                                    className="h-4 w-4 rounded border-2 border-border-soft text-primary focus:ring-primary caret-primary cursor-pointer checked:bg-primary"
                                                                    checked={permissionFilterIds.includes(p.id)}
                                                                    onChange={() => togglePermissionFilter(p.id)}
                                                                />
                                                                <span className="text-xs font-bold text-text-soft group-hover:text-text-main transition-colors">{p.label}</span>
                                                            </label>
                                                        ))}
                                                    </div>
                                                ))}
                                            ...
                                            {permissionFilterIds.length > 0 && (
                                                <div className="px-6 pt-4 border-t border-border-soft mt-2">
                                                    <button
                                                        onClick={() => setPermissionFilterIds([])}
            ...
            {isModalOpen && editingUser && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-[500px] bg-white rounded-[2rem] shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col">
                        {/* Modal Header */}
                        <div className="px-10 py-8 space-y-2 flex-shrink-0">
                            <button
                        ...
                        {/* Modal Body */}
                        <div className="px-10 pb-8 space-y-8 overflow-y-auto custom-scrollbar flex-1" style={{ maxHeight: 'calc(100vh - 300px)' }}>
                            {/* Role Selection */}
                        ...
                        {/* Modal Footer */}
                        <div className="p-8 bg-gray-50/50 border-t border-border-soft flex items-center justify-end gap-3 flex-shrink-0">
                            <button
                                onClick={() => setIsModalOpen(false)}
